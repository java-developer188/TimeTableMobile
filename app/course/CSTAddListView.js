...
export default class CSTAddListView extends Component {
    ...
    renderRow = (rowData, rowID) => {
        ...
        if (SessionManager.getSessionValue(Constants.FINAL_SELECTED_COURSES) !== null) {
            selected = JSON.parse(SessionManager.getSessionValue(Constants.FINAL_SELECTED_COURSES).CST);
            ...
            SessionManager.setSessionValue(Constants.FINAL_SELECTED_COURSES, []);
        }
    ...
    addCourse(cst){
        ...
        this.webservicemanager.callWebService("courses/add", "", params, (response) => {
    ...
    removeCourse(cst){
        ...
        this.webservicemanager.callWebService("courses/remove", "", params, (response) => {
            this.handleResponse(response);
        });
    }

    handleResponse(response){
        if (response.CST !== null && response.CST.length > 0) {
           // alert(JSON.stringify(response))
            SessionManager.setSessionValue(Constants.FINAL_SELECTED_COURSES, response);
            //this.props.navigator.popToRoute(Screens.AddRemoveByBatchScreen);
        }
    }
