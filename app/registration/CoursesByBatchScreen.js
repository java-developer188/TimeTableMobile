...
export default class CoursesByBatchScreen extends Component {
    ...
    callCoursesByBatchScreen(batch) {
        ...
        this.props.navigator.push(Screens.RegisterCourseContainer);
    }
    ...
    showSelectedCoursesScreen() {
        if (SessionManager.getSessionValue(Constants.SELECTED_COURSES) !== null) {
            this.props.navigator.push(Screens.SelectedCoursesContainer);
        }
        else {
    ...
    registrationWebServiceCall() {
        var student;
        if (SessionManager.getSessionValue(Constants.REGISTRATION_DATA) !== null) {
            student = SessionManager.getSessionValue(Constants.REGISTRATION_DATA);
            if (SessionManager.getSessionValue(Constants.SELECTED_COURSES) !== null) {
                for (var value of SessionManager.getSessionValue(Constants.SELECTED_COURSES)) {
                    student.courses.push(value.id);
                }
            }
        }
        this.webservicemanager.callWebService("register", "", student, (response) => {
    ...
    handleWebServiceCallResponse(response) {
        ...
        Alert.alert('Registration Successful ',
            ...
            [
                {
                    text: 'OK', onPress: () => {
                    this.props.navigator.popToRoute(Screens.LoginScreen)
                }
                },
            ],
            {cancelable: false}
    ...
    handleFailedRegistration(response) {
        Alert.alert('Registration Failed ',
            'Unfortunately your registration has been failed. Please try again',
            [
                {
                    text: 'OK', onPress: () => {
                    this.props.navigator.popToRoute(Screens.RegistrationScreen)
                }
                },
            ],
            {cancelable: false}
